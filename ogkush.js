var dataHelper = (function () {
  ...
  function toFormatedNumber(value, precision = null, units = false) {
    const commaSeparator = ["us", "en", "ro", "tw"];
    let locale = document.querySelector("#pixelzirkus").getAttribute("data-default-language");
  ...
  class OGInfinity {
    constructor() {
      ...
      this.universe = document.querySelector("#pixelzirkus").getAttribute("data-default-server-id"); // window.location.host.replace(/\D/g, "")
    ...
    async updateServerSettings(force = false) {
      ...
      let settingsUrl = `http://${this.host}/api/serverData.xml`;
    ...
    fetchAndConvertRC(messageId) {
      const url = `http://${this.host}/game/index.php?page=messages&messageId=${messageId}&tabid=21&ajax=1`;
      return fetch(url)
        .then((rep) => rep.text())
        .then((str) => {
    ...
    generateHiscoreLink(playerid) {
      return `http://${this.host}/game/index.php?page=highscore&searchRelId=${playerid}`;
    }
    ...
    playerSearch(show, name) {
      let renderPlayerInfo = (player) => {
        ...
        let name = `<span>${player.name}</span> <span class="${this.getPlayerStatus(
          player.status
        )}"></span>\n                  <a target="_self"\n                    href="http://${this.host}/game/index.php?page=highscore&searchRelId=${
          player.id
      ...
      if (this.page == "fleetdispatch" && fleetDispatcher.shipsOnPlanet.length == 0) {
        ...
        neededShips.appendChild(
          this.createDOM(
            ...
            `<a tech-id="202" class="ogl-option noShips ogl-fleet-ship ogl-fleet-202" href="http://${
              window.location.host
            }${
              window.location.pathname
            }?page=ingame&component=shipyard&cp=${planetId}&techId202=${sc}"></a><span>${toFormatedNumber(sc, 0)}</span>
            <a tech-id="203" class="ogl-option noShips ogl-fleet-ship ogl-fleet-203" href="http://${
              window.location.host
            }${
              window.location.pathname
            }?page=ingame&component=shipyard&cp=${planetId}&techId203=${lc}"></a><span>${toFormatedNumber(lc, 0)}</span>
            <a tech-id="219" class="ogl-option noShips ogl-fleet-ship ogl-fleet-219" href="http://${
              window.location.host
            }${
              window.location.pathname
            }?page=ingame&component=shipyard&cp=${planetId}&techId219=${pf}"></a><span>${toFormatedNumber(pf, 0)}</span>
            <a tech-id="209" class="ogl-option noShips ogl-fleet-ship ogl-fleet-209" href="http://${
              window.location.host
            }${
              window.location.pathname
    ...
    async getSelectedLifeform(planetId) {
      ...
      return fetch(
        `http://${this.host}/game/index.php?page=ingame&component=lfsettings&cp=${planetId}`,
    ...
    async getLifeformBonus(planetId) {
      ...
      return fetch(
        `http://${this.host}/game/index.php?page=ingame&component=bonussettings&cp=${planetId}`,
        { signal: abortController.signal }
      )
        .then((rep) => rep.text())
        .then((str) => {
    ...
    async getEmpireInfo() {
      ...
      return fetch(
        `http://${this.host}/game/index.php?page=standalone&component=empire`,
        { signal: abortController.signal }
      )
        .then((rep) => rep.text())
        .then((str) => {
          ...
          if (hasMoon) {
            return fetch(
              `http://${this.host}/game/index.php?page=standalone&component=empire&planetType=1`,
              { signal: abortController.signal }
            )
              .then((rep) => rep.text())
              .then((str) => {
    ...
    updateresourceDetail() {
      ...
      for (const [coords, details] of Object.entries(flyingDetails)) {
        ...
        if (res > 0) {
          let href = `http://${this.host}/game/index.php?page=ingame&amp;component=galaxy&galaxy=${coords.split(":")[0]}&system=${
            coords.split(":")[1]
    ...
    sendMessage(id) {
      if (this.tchat) {
        ...
        document.location = `http://${this.host}/game/index.php?page=chat&playerId=${id}`;
      }
    }
  
    generateIgnoreLink(playerId) {
      return `http://${this.host}/game/index.php?page=ignorelist&action=1&id=${playerId}`;
    }
  
    generateBuddyLink(playerId) {
      return `http://${this.host}/game/index.php?page=ingame&component=buddies&action=7&id=${playerId}&ajax=1`;
    }
    ...
    spyTable() {
      if (this.page == "fleetdispatch" && this.mode == 4) {
        let link = "http://" + window.location.host + window.location.pathname + "?page=messages";
    ...
    sortTable(arr) {
      ...
      arr.forEach(async (report, index) => {
        ...
        ship.appendChild(
          createDOM(
            "a",
            { href: "http://" + window.location.host + window.location.pathname + fleetLink },
          ...
          total.addEventListener("click", (e) => {
            ...
            for (let round = 1; round < renta.length; round++) {
              ...
              extraShip.appendChild(
                createDOM(
                  "a",
                  { href: "http://" + window.location.host + window.location.pathname + extraFleetLink },
    ...
    deleteMSg(msgId) {
      ...
      return fetch(`http://${this.host}/game/index.php?page=messages`, {
        method: "POST",
        body: requestData,
      })
        .then((response) => response.json())
        ...
        close.addEventListener("click", () => {
          if (this.json.welcome) {
            ...
            if (this.playerClass == PLAYER_CLASS_NONE) {
              window.location.href = `http://${this.host}/game/index.php?page=ingame&component=characterclassselection`;
            } else {
              window.location.href = `http://${this.host}/game/index.php?page=ingame&component=overview`;
            }
          }
          overlay.classList.remove("ogl-active");
        });
    ...
    selectBestCargoShip(preveredShipId = null) {
      if (fleetDispatcher.currentPage == "fleet1" && fleetDispatcher.shipsOnPlanet.length != 0) {
        ...
        let metal = Number(metalFiller.value.split(separatorByLang).join(""));
        if (metal > metalAvailable) metalFiller.value = toFormatedNumber(metalAvailable, 0);
        let crystal = Number(crystalFiller.value.split(separatorByLang).join(""));
        if (crystal > crystalAvailable) crystalFiller.value = toFormatedNumber(crystalAvailable, 0);
        let deut = Number(deutFiller.value.split(separatorByLang).join(""));
        if (deut > deutAvailable)
          deutFiller.value = toFormatedNumber(Math.max(0, deutAvailable - fleetDispatcher.getConsumption()), 0);
        let resources =
          Math.min(metal, metalAvailable) + Math.min(crystal, crystalAvailable) + Math.min(deut, deutAvailable);
