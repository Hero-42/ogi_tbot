class DataHelper {
    ...
    _updateHighscore(players) {
      ...
      types.forEach((type, index) => {
        let p = this._fetchXML(
          `http://${this.host}/api/highscore.xml?category=1&type=` + index
        ).then((xml) => {
    ...
    _updatePlayers(players) {
      return fetch(`http://${this.host}/api/players.xml`)
        .then((rep) => rep.text())
        .then((str) => new window.DOMParser().parseFromString(str, "text/xml"))
        .then((xml) => {
    ...
    _updatePlanets(players) {
      return fetch(`http://${this.host}/api/universe.xml`)
        .then((rep) => rep.text())
        .then((str) => new window.DOMParser().parseFromString(str, "text/xml"))
        .then((xml) => {
          let update = new Date(Number(xml.children[0].getAttribute("timestamp")) * 1e3);
    ...
    _updateAlliances(players) {
      return fetch(`http://${this.host}/api/alliances.xml`)
        .then((rep) => rep.text())
        .then((str) => new window.DOMParser().parseFromString(str, "text/xml"))
        .then((xml) => {
